...
def get_frame_values(video_files, percentile_value):
    '''
    ...
    for fname in tqdm(video_files): #Loop through each video file (each filename)
        ...
        for frame in vid.iter_data(): #Loop through each frame in the video
            temp_video_data.append(np.percentile(frame,percentile_value)) #Calculate and save the desired percentile value from the frame
        ...
        video_data.append(value_dict) #Save the values from all the frames into the dictionary, indexed the path to the video file in question
    return video_data
...
if __name__ == "__main__":
    ...
    #############################################################
    # INPUT - Define folder video files are in and which percentile value to use
